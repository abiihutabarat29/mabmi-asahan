<!DOCTYPE html>
<html lang="en">

<head>
    <title>:: Administrator | Mabmi ::</title>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no" />

    <link href="https://fonts.googleapis.com/css?family=Lato:300,400,700&display=swap" rel="stylesheet" />

    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css" />

    <!-- Icons. Uncomment required icon fonts -->
    <link rel="stylesheet" href="{{ asset('/') }}dist/assets/fonts/boxicons.css" />
    <link rel="stylesheet" href="{{ asset('/') }}dist/assets/css/core.css" class="template-customizer-core-css" />
    <link rel="stylesheet" href="{{ asset('/') }}dist/assets/css/theme-default.css"
        class="template-customizer-theme-css" />
    <link rel="icon" href="{{ url('dist/assets/img/logo-mabmi.png') }}" type="image/x-icon" />

    <script src="{{ asset('/') }}dist/assets/js/helpers.js"></script>

    <link rel="stylesheet" href="{{ asset('/') }}dist/assets/css/style.css" />
</head>

<body>
